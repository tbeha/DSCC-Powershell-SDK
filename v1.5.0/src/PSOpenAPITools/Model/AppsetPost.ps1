...
AppsetPost<PSCustomObject>
#>

function Initialize-AppsetPost {
    [CmdletBinding()]
    Param (
        ...
        [ValidateSet("PARENT_TIMESTAMP", "PARENT_SEC_SINCE_EPOCH")]
        [PSCustomObject]
        ${VvNamePattern}
    )

    Process {
        'Creating PSCustomObject: PSOpenAPITools => AppsetPost' | Write-Debug
...
Convert from JSON to AppsetPost<PSCustomObject>
...
AppsetPost<PSCustomObject>
#>
function ConvertFrom-JsonToAppsetPost {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => AppsetPost' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in AppsetPost
        ...
        If ([string]::IsNullOrEmpty($Json) -or $Json -eq "{}") { # empty json
            throw "Error! Empty JSON cannot be serialized due to the required property 'snapshotName' missing."
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "snapshotName"))) {
            throw "Error! JSON cannot be serialized due to the required property 'snapshotName' missing."
        } else {
