#
# Data Services Cloud Console API
# Data Services Cloud Console API
# Version: 1.4.0
# Generated by OpenAPI Generator: https://openapi-generator.tech
#

<#
.SYNOPSIS

No summary available.

.DESCRIPTION

Request body to create replication partners

.PARAMETER ReplicationPartners
No description available.
.OUTPUTS

CreateReplicationPartnersInput<PSCustomObject>
#>

function Initialize-CreateReplicationPartnersInput {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipelineByPropertyName = $true)]
        [PSCustomObject[]]
        ${ReplicationPartners}
    )

    Process {
        'Creating PSCustomObject: PSOpenAPITools => CreateReplicationPartnersInput' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        if ($null -eq $ReplicationPartners) {
            throw "invalid value for 'ReplicationPartners', 'ReplicationPartners' cannot be null."
        }


        $PSO = [PSCustomObject]@{
            "replicationPartners" = ${ReplicationPartners}
        }


        return $PSO
    }
}

<#
.SYNOPSIS

Convert from JSON to CreateReplicationPartnersInput<PSCustomObject>

.DESCRIPTION

Convert from JSON to CreateReplicationPartnersInput<PSCustomObject>

.PARAMETER Json

Json object

.OUTPUTS

CreateReplicationPartnersInput<PSCustomObject>
#>
function ConvertFrom-JsonToCreateReplicationPartnersInput {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )

    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => CreateReplicationPartnersInput' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in CreateReplicationPartnersInput
        $AllProperties = ("replicationPartners")
        foreach ($name in $JsonParameters.PsObject.Properties.Name) {
            if (!($AllProperties.Contains($name))) {
                throw "Error! JSON key '$name' not found in the properties: $($AllProperties)"
            }
        }

        If ([string]::IsNullOrEmpty($Json) -or $Json -eq "{}") { # empty json
            throw "Error! Empty JSON cannot be serialized due to the required property 'replicationPartners' missing."
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "replicationPartners"))) {
            throw "Error! JSON cannot be serialized due to the required property 'replicationPartners' missing."
        } else {
            $ReplicationPartners = $JsonParameters.PSobject.Properties["replicationPartners"].value
        }

        $PSO = [PSCustomObject]@{
            "replicationPartners" = ${ReplicationPartners}
        }

        return $PSO
    }

}

