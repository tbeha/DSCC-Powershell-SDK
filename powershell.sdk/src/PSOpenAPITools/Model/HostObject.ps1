...
Host Update or Delete progress status. Possible status are: Update_In_Progress,Update_Success,Update_Failed,Delete_In_Progress,Delete_Failed,Not_Applicable. `Filter`
.PARAMETER Fqdn
Fully qualified domain name of the host.
.PARAMETER Generation
A monotonically increasing value. This value updates when the resource is updated and can be used as a short way to determine if a resource has changed or which of two different copies of a resource is more up to date.
.PARAMETER HostGroups
Host group to which the host belongs to. `Filter` by hostGroupId.
.PARAMETER Id
Identifier for host. `Filter`
.PARAMETER Initiators
Host initiator list this host is associated with. `Filter` by initiatorId.
.PARAMETER IpAddress
IP address of the host.
.PARAMETER IsMergable
Indicates whether host has a duplicate
.PARAMETER Location
location.
...
function Initialize-HostObject {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => HostObject' | Write-Debug
...
Convert from JSON to HostObject<PSCustomObject>
...
HostObject<PSCustomObject>
#>
function ConvertFrom-JsonToHostObject {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => HostObject' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in HostObject
