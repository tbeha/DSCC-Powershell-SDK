...
Import-Module -Name '.\powershell.sdk\src\PSOpenAPITools' -SkipEditionCheck #-Verbose
...
try {
    ...
    Write-Host ("Exception occurred when calling Invoke-SystemsList: {0}" -f ($_ | ConvertFrom-Json))
    Write-Host ("Response headers: {0}" -f ($_.Exception.Response.Headers | ConvertTo-Json))
}
$SystemId = $SystemIds.CTC_Alletra9060
try{
	$Volume = (Invoke-VolumesList -Filter 'name eq DSCC\-API\-Vol').items[0]
} catch {
...
$UnExportVlun = Initialize-UnExportVlun -HostGroupIds $HostGroupId # UnExportVlun |
try{
...
try {
    $Result = Invoke-VolumeDelete -SystemId $SystemId -Id $Volume.id 
